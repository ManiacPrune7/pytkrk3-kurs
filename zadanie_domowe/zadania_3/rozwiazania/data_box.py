...
class DataBox:
    ...
    def add_data(self, data):
        if type(data) not in (int, float, list):
            raise BadDataTypeError("Type of data is not one of <int, float, list>!!!")
    ...
    def count_geometric_average(self):
        return reduce(lambda x, y: x * y, self.data) ** (1 / self.amount_of_data)

    def get_modal(self):
        modal = self.data[0]

        for data in set(self.data):
            if self.data.count(data) > modal:
                modal = self.data.count(data)
